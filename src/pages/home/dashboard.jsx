...
export default function Dashboard() {
  ...
  function getProgressColor(percent) {
    if (percent >= 100) return "bg-[#b4182D]";
    if (percent >= 50) return "bg-[#ED9E59]";
    return "bg-teal-700";
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 text-accent text-center">Spending Progress Overview</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {scheduleProgresses.map((progress) => {
            ...
            return (
              <div key={progress._id} className="bg-white p-5 rounded-xl shadow-lg hover:shadow-xl transition">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <FaRegCalendarAlt className="text-accent" />
                    <span className={`text-sm px-3 py-1 rounded-full font-semibold ${badgeClass}`}>
                ...
                <p className="mt-2 text-sm text-gray-700">
                  <span className="font-semibold text-accent">LKR {progress.totalSpent.toFixed(2)}</span> of{" "}
                  <span className="text-gray-600">LKR {progress.targetAmount.toFixed(2)}</span>
                </p>
              </div>
            );
          })}
